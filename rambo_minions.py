...
def create_enemies(board, amount_of_enemies):
    '''
    '''
    ...
    while amount_of_enemies:
        ...
        if board[y-1][x-1] not in rambo.OBSTACLES:
...
def move_enemies(board, positions):
    new_positions = []
    for x, y in positions:
        direction = random.randint(1,4)
#        print(x, y)
        if direction == 1 and board[y-2][x-1] not in rambo.OBSTACLES:
            new_positions.append((x, y-1))
        elif direction == 2 and board[y][x-1] not in rambo.OBSTACLES:
            new_positions.append((x, y+1))
        elif direction == 3 and board[y-1][x-2] not in rambo.OBSTACLES:
            new_positions.append((x-1, y))
        elif direction == 4 and board[y-1][x] not in rambo.OBSTACLES:
            new_positions.append((x+1, y))
        else:
